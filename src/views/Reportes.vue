        ...
        <ion-card>
          ...
          EN VEZ DE USAR ESTO, CREA TUS PROPIAR REGLAS "MEDIA" EN CSS -->
            ...
            <form @submit.prevent="handleSubmit">
              <ion-row>
                <ion-col>
                  <ion-input v-model="form_data.loan" fill="outline" label-placement="floating" label="Codigo"
              ...
              <ion-row>
                <ion-col>
                  <ion-input v-model="form_data.interest" fill="outline" label-placement="floating" label="Semestre"
...
const handleSubmit = async () => {
  ...
  const alert = await alertController.create({
    header: 'SUCCESS',
    message: `Cuota: ${quo.toFixed(2)} ===> Codigo: ${amnt.toFixed(2)/100}`,
    buttons: ['ok'],
  });
  await alert.present();
